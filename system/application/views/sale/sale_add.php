 ...
 */
 ...
 <center>
	 <table  width="80%" class="mytable"> 
		 ...
		 <td><input type="text" id="product_real_name" readonly="true"  name="<?php echo "detail[product_real_name][$k]";?>"  value="<?php echo set_value("detail[product_real_name][$k]",$v['product_real_name']);?>" ></td>
		 <td>
		   ...
		   <td>
				<input type="text" id="product_price"   name="detail[product_price][<?php echo $k;?>]" value="<?php echo set_value("detail[product_price][$k]",$v['product_price']);?>">
		  ...
		  <td>		
			...
			<a href="javascript:;"  target="_selft" onclick="core_copy($(this),'#product_id,#quantity,#product_name,#detail_id','copy_extend')" class="red_font">复制</a>
			<a href="javascript:;"  id="delete_link"  class="red_font  <?php if($k==0):?>hide<?php endif;?>"    target="_selft" onclick="core_drop($(this))" >删除</a>
...
<script language="javascript">
	...
	function ajax_product(obj){	
		...
		$.getJSON(base_url, function(json){	
		 	parent.find('#product_real_name').val(json.product_real_name);
		});		 
	}
	
	$('#form1').submit(function(){		
			$('#form_submit_flag').val("1");
	});
	
	window.onbeforeunload = function(e){
		if($('#form_submit_flag').val()!="1"){
